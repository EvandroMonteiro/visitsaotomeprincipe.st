<template>
  <main>
    ...
    <ListCategorias :categorias="categorias" :baseSlug="'/o-que-fazer/' + $route.params.ilhas" :items="filterLocais(locais)" />
    <ModuloInformacoes />
  </main>
</template>
    ...
    filterLocais(items){
      ...
      for (const key in items) {
        if (Object.hasOwnProperty.call(items, key)) {
          ...
          if (element.regiao.ilha.slug === this.slugIlha) { //valida se pertence à ilha
            if (element.categorias.length) {
              for (const secondKey in element.categorias) {
                if (Object.hasOwnProperty.call(element.categorias, secondKey)) { 
                  ...
                  if (secondElement.slug == this.slug) { //valida se pertence à categoria
